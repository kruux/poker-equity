...
fn test_hand_from_str() -> Result<(), PokerError> {
    // Test without spaces
    let hand = Hand::from_str("AhKdQc2s3d")?;
    assert_eq!(hand.num_cards(), 5);

    // Test with spaces
    let hand_with_spaces = Hand::from_str("Ah Kd Qc 2s 3d ")?;
    assert_eq!(hand.num_cards(), 5);
...
fn hand_from_str_with_too_many_cards() -> Result<(), PokerError> {
    assert!(matches!(
        Hand::from_str("AhKdQc2s3d4h").unwrap_err(),
...
fn test_hand_comparison() -> Result<(), PokerError> {
    // 75432 vs 76432
    let hand1 = Hand::from_str("7d5h4c3s2h")?;
    let hand2 = Hand::from_str("7c6s4h3d2c")?;
...
fn test_two_pair_order() -> Result<(), PokerError> {
    ...
    let hand1 = Hand::from_str("7c7h2s2dAh")?;
    assert!(matches!(
        hand1.evaluate(),
        HandRank::TwoPair(Rank::Seven, Rank::Two, Rank::Ace)
    ));

    // Test different order in input gives same result
    let hand2 = Hand::from_str("2s2d7c7hAh")?;
    assert!(matches!(
        hand2.evaluate(),
        HandRank::TwoPair(Rank::Seven, Rank::Two, Rank::Ace)
    ));
...
fn test_hand_evaluation() -> Result<(), PokerError> {
    ...
    let straight_flush = Hand::from_str("7h6h5h4h3h")?;
    assert!(matches!(
    ...
    // Test four of a kind
    let quads = Hand::from_str("7c7h7s7dAh")?;
    assert!(matches!(
        quads.evaluate(),
        HandRank::FourOfAKind(Rank::Seven)
    ));

    // Test full house
    let full_house = Hand::from_str("7c7h7sAhAd")?;
    assert!(matches!(
        full_house.evaluate(),
    ...
    // Test flush
    let flush = Hand::from_str("Ah7h5h4h2h")?;
    assert!(matches!(flush.evaluate(), HandRank::Flush(_)));

    // Test straight (remember A2345 is not a straight in 2-7!)
    let straight = Hand::from_str("7c6h5s4d3h")?;
    assert!(matches!(
        straight.evaluate(),
        HandRank::Straight(Rank::Seven)
    ));

    // Test three of a kind
    let trips = Hand::from_str("7c7h7sAh2d")?;
    assert!(matches!(
        trips.evaluate(),
        HandRank::ThreeOfAKind(Rank::Seven)
    ));

    // Test two pair
    let two_pair = Hand::from_str("7c7h2s2dAh")?;
    ...
    // Test one pair
    let pair = Hand::from_str("7c7h3s4d2h")?;
    assert!(matches!(pair.evaluate(), HandRank::Pair(Rank::Seven, _)));

    // Test high card (best possible 2-7 hand: 7532A)
    let high_card = Hand::from_str("7c5h3s2dAh")?;
    assert!(matches!(high_card.evaluate(), HandRank::HighCard(_)));
...
fn test_not_straight() -> Result<(), PokerError> {
    ...
    let wheel = Hand::from_str("Ah2h3h4h5h")?;
    assert!(matches!(wheel.evaluate(), HandRank::Flush(_))); // Should be evaluated as just a flush

    let wheel_mixed = Hand::from_str("Ac2h3d4s5h")?;
    assert!(matches!(wheel_mixed.evaluate(), HandRank::HighCard(_))); // Just a high card hand
...
fn test_kickers_order() -> Result<(), PokerError> {
    ...
    let pair_with_kickers = Hand::from_str("7c7hAh5h2d")?;
    if let HandRank::Pair(rank, kickers) = pair_with_kickers.evaluate() {
...
fn test_straight_flush_comparison() -> Result<(), PokerError> {
    ...
    let hand1 = Hand::from_str("7h6h5h4h3h")?; // 7-high straight flush
    let hand2 = Hand::from_str("8h7h6h5h4h")?; // 8-high straight flush
    assert!(hand1 > hand2);
    Ok(())
}
...
fn test_four_of_kind_comparison() -> Result<(), PokerError> {
    ...
    let hand1 = Hand::from_str("2c2h2s2dAh")?;
    let hand2 = Hand::from_str("3c3h3s3dAh")?;
    assert!(hand1 > hand2);
    Ok(())
}
...
fn test_full_house_comparison() -> Result<(), PokerError> {
    ...
    let hand1 = Hand::from_str("2c2h2sAhAd")?; // 2s full of Aces
    let hand2 = Hand::from_str("3c3h3sKhKd")?; // 3s full of Kings
    assert!(hand1 > hand2);

    // Same trips, different pairs
    let hand3 = Hand::from_str("2c2h2sKhKd")?; // 2s full of Kings
    assert!(hand1 < hand3);
...
fn test_flush_comparison() -> Result<(), PokerError> {
    ...
    let hand1 = Hand::from_str("7h5h4h3h2h")?;
    let hand2 = Hand::from_str("7h6h4h3h2h")?;
    assert!(hand1 > hand2);
    Ok(())
}
...
fn test_straight_comparison() -> Result<(), PokerError> {
    ...
    let hand1 = Hand::from_str("6c5h4s3d2h")?;
    let hand2 = Hand::from_str("7c6h5s4d3h")?;
    assert!(hand1 > hand2);
    Ok(())
}
...
fn test_three_of_kind_comparison() -> Result<(), PokerError> {
    ...
    let hand1 = Hand::from_str("2c2h2sAhKd")?;
    let hand2 = Hand::from_str("3c3h3sAhKd")?;
    assert!(hand1 > hand2);
    Ok(())
}
...
fn test_two_pair_comparison() -> Result<(), PokerError> {
    ...
    let hand1 = Hand::from_str("3c3h2s2dAh")?;
    let hand2 = Hand::from_str("4c4h2s2dKh")?;
    assert!(hand1 > hand2);

    // Same high pair, compare low pair
    let hand3 = Hand::from_str("3c3h4s4dQh")?;
    assert!(hand1 > hand3);

    // Same pairs, compare kicker
    let hand4 = Hand::from_str("3c3h2s2dKh")?;
    assert!(hand1 < hand4);
...
fn test_pair_comparison() -> Result<(), PokerError> {
    ...
    let hand1 = Hand::from_str("2c2h7s5d3h")?;
    let hand2 = Hand::from_str("3c3h7s5d4h")?;
    assert!(hand1 > hand2);

    // Same pair, compare kickers
    let hand3 = Hand::from_str("2c2h8s5d3h")?;
    // 753 beats 853
    assert!(hand1 > hand3);
...
fn test_high_card_comparison() -> Result<(), PokerError> {
    ...
    let hand1 = Hand::from_str("7c5h3s2dAh")?; // 7532A
    let hand2 = Hand::from_str("7c6h3s2dAh")?; // 7632A
    assert!(hand1 > hand2);
    Ok(())
}
...
fn test_different_hand_types() -> Result<(), PokerError> {
    ...
    let high_card = Hand::from_str("7c5h3s2dAh")?;
    let pair = Hand::from_str("2c2h7s5d3h")?;
    assert!(high_card > pair);

    // Pair beats two pair
    let two_pair = Hand::from_str("2c2h3s3dAh")?;
