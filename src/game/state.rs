...
impl GameState {
    pub fn new(max_players: usize, player_names: Vec<&str>) -> Self {
        let deck = Deck::new();
    ...
    pub fn find_player(&self, name: &str) -> Result<&Player, GameError> {
        self.players
            .iter()
            .find(|p| p.name() == name)
    ...
    fn find_player_mut(&mut self, name: &str) -> Result<&mut Player, GameError> {
        self.players
            .iter_mut()
            .find(|p| p.name() == name)
            .ok_or(GameError::PlayerNotFound(name.to_string()))
