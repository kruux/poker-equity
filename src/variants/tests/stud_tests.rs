...
use crate::{
    ...
    error::PokerError,
    hand::Hand,
    variants::{rankings::HighHandRank, Stud},
};

#[test]
fn test_wheel_straight() {
    let hand = Hand::<Stud>::from_str(Stud, "As 2h 3d 4c 5h 7s 8d").unwrap();
...
fn test_best_of_seven() {
    ...
    let hand = Hand::<Stud>::from_str(Stud, "2h 3d 4c 5h 6s 7s Ad").unwrap();
    assert_eq!(hand.evaluate(), HighHandRank::Straight(Rank::Seven));
}

#[test]
fn test_best_flush() {
    let hand = Hand::<Stud>::from_str(Stud, "2h 4h 6h 8h Th Jh 3s").unwrap();
...
fn test_not_straight_flush() {
    ...
    let hand = Hand::<Stud>::from_str(Stud, "Ah Kh Qh Jh 2h 9c Td").unwrap();
...
fn test_actual_straight_flush() {
    ...
    let hand = Hand::<Stud>::from_str(Stud, "9h Th Jh Qh Kh 2c 3c").unwrap();
...
fn test_straight_flush() {
    ...
    let hand = Hand::<Stud>::from_str(Stud, "9h Th Jh Qh Kh 2c 3d").unwrap();
    assert_eq!(hand.evaluate(), HighHandRank::StraightFlush(Rank::King));

    // Wheel straight flush
    let hand = Hand::<Stud>::from_str(Stud, "Ah 2h 3h 4h 5h Kc Qd").unwrap();
    assert_eq!(hand.evaluate(), HighHandRank::StraightFlush(Rank::Five));

    // A-6 straight flush (should be 6 high)
    let hand = Hand::<Stud>::from_str(Stud, "Ah 2h 3h 4h 5h 6h Kd").unwrap();
...
fn test_four_of_a_kind() {
    let hand = Hand::<Stud>::from_str(Stud, "Ah Ac Ad As Kh Qc Jd").unwrap();
    assert_eq!(
        hand.evaluate(),
    ...
    let hand = Hand::<Stud>::from_str(Stud, "7h 7c 7s 7d 8h 9h Th").unwrap();
    assert_eq!(
        hand.evaluate(),
...
fn test_full_house() {
    // Regular full house
    let hand = Hand::<Stud>::from_str(Stud, "Ah Ac Ad Kh Kc 2c 3d").unwrap();
    assert_eq!(
        hand.evaluate(),
    ...
    // Two sets of trips
    let hand = Hand::<Stud>::from_str(Stud, "Ah Ac Ad Kh Kc Kd Qc").unwrap();
    assert_eq!(
        hand.evaluate(),
        HighHandRank::FullHouse(Rank::Ace, Rank::King)
    );
    ...
    // Trips and two pairs
    let hand = Hand::<Stud>::from_str(Stud, "Ah Ac Ad Kh Kc Qh Qc").unwrap();
    assert_eq!(
        hand.evaluate(),
        HighHandRank::FullHouse(Rank::Ace, Rank::King)
    );
...
fn test_flush() {
    // Basic flush
    let hand = Hand::<Stud>::from_str(Stud, "Ah Kh Qh Jh 9h 2c 3d").unwrap();
    if let HighHandRank::Flush(ranks) = hand.evaluate() {
    ...
    // Six card flush
    let hand = Hand::<Stud>::from_str(Stud, "2h Kh Qh Jh 9h Ah 3d").unwrap();
    if let HighHandRank::Flush(ranks) = hand.evaluate() {
        assert_eq!(ranks.len(), 5);
        assert_eq!(ranks[0], Rank::Ace);
    ...
    // Seven card flush
    let hand = Hand::<Stud>::from_str(Stud, "Ah Kh Qh Jh 2h 7h 9h").unwrap();
    if let HighHandRank::Flush(ranks) = hand.evaluate() {
        assert_eq!(ranks.len(), 5);
        assert_eq!(ranks[0], Rank::Ace);
        assert_eq!(ranks[4], Rank::Nine); // Should still take best 5
...
fn test_straight() {
    // Regular straight
    let hand = Hand::<Stud>::from_str(Stud, "9c Th Jd Qh Kc 2c 3d").unwrap();
    assert_eq!(hand.evaluate(), HighHandRank::Straight(Rank::King));

    // Wheel straight (A2345)
    let hand = Hand::<Stud>::from_str(Stud, "Ac 2h 3d 4h 5c Kc Qd").unwrap();
    assert_eq!(hand.evaluate(), HighHandRank::Straight(Rank::Five));

    // A-6 straight
    let hand = Hand::<Stud>::from_str(Stud, "Ac 2h 3d 4h 5c 6d Kd").unwrap();
    assert_eq!(hand.evaluate(), HighHandRank::Straight(Rank::Six));

    // Multiple straights - should find highest
    let hand = Hand::<Stud>::from_str(Stud, "5c 6h 7d 8h 9c Th Jd").unwrap();
...
fn test_three_of_a_kind() {
    ...
    let hand = Hand::<Stud>::from_str(Stud, "Ah Ac Ad Qh Jc 9c 2d").unwrap();
    ...
    let hand = Hand::<Stud>::from_str(Stud, "7h 7c 7d 8h 9c Tc 2d").unwrap();
    if let HighHandRank::ThreeOfAKind(rank, kickers) = hand.evaluate() {
        assert_eq!(rank, Rank::Seven);
    ...
    let hand = Hand::<Stud>::from_str(Stud, "7h 7c 7d 2h 3h 4h 9c").unwrap();
    if let HighHandRank::ThreeOfAKind(rank, kickers) = hand.evaluate() {
        assert_eq!(rank, Rank::Seven);
...
fn test_two_pair() {
    // Regular two pair
    let hand = Hand::<Stud>::from_str(Stud, "Ah Ac 5h 5c Qc Jc 2d").unwrap();
    assert_eq!(
        hand.evaluate(),
    ...
    let hand = Hand::<Stud>::from_str(Stud, "Kh Kc Ah Ac Qh Qc 2d").unwrap();
    assert_eq!(
        hand.evaluate(),
    ...
    let hand = Hand::<Stud>::from_str(Stud, "Ah Ac Qs Js Kh Kc 3d").unwrap();
    assert_eq!(
        hand.evaluate(),
        HighHandRank::TwoPair(Rank::Ace, Rank::King, Rank::Queen)
    );
...
fn test_pair() {
    // Basic pair
    let hand = Hand::<Stud>::from_str(Stud, "Ah Ac Kh Qc Jc 9c 7d").unwrap();
    ...
    let hand = Hand::<Stud>::from_str(Stud, "7h 7c 8h 9c Tc 2c 3d").unwrap();
    ...
    let hand = Hand::<Stud>::from_str(Stud, "7h 7c 2h 3h 4h 9c Td").unwrap();
    if let HighHandRank::Pair(rank, _) = hand.evaluate() {
        assert_eq!(rank, Rank::Seven);
    } else {
        panic!("Should be a pair!");
...
fn test_high_card() {
    // Basic high card
    let hand = Hand::<Stud>::from_str(Stud, "Ah Kc Qh Jc 9c 7c 2d").unwrap();
    ...
    let hand = Hand::<Stud>::from_str(Stud, "Ah 3c 4h 5c 6c 8c 9d").unwrap();
    if let HighHandRank::HighCard(ranks) = hand.evaluate() {
        assert_eq!(ranks[0], Rank::Ace);
    } else {
        panic!("Should be high card!");
    ...
    let hand = Hand::<Stud>::from_str(Stud, "Ah 3h 4h 5h 7c 8c 9d").unwrap();
    if let HighHandRank::HighCard(ranks) = hand.evaluate() {
        assert_eq!(ranks[0], Rank::Ace);
    } else {
        panic!("Should be high card!");
...
fn test_two_pair_edge_cases() {
    ...
    let hand = Hand::<Stud>::from_str(Stud, "Kh Kc Qh Qc Jh Jc 2d").unwrap();
    assert_eq!(
        hand.evaluate(),
    ...
    let hand = Hand::<Stud>::from_str(Stud, "Qh Qc Kh Kc Jh Jc 2d").unwrap();
    assert_eq!(
        hand.evaluate(),
        HighHandRank::TwoPair(Rank::King, Rank::Queen, Rank::Jack)
    );
    ...
    let hand = Hand::<Stud>::from_str(Stud, "Jh Jc Kh Kc Qh Qc 2d").unwrap();
    assert_eq!(
        hand.evaluate(),
        HighHandRank::TwoPair(Rank::King, Rank::Queen, Rank::Jack)
    );
    ...
    let hand = Hand::<Stud>::from_str(Stud, "Kh Kc Qh Qc Ah 9c 7d").unwrap();
    assert_eq!(
        hand.evaluate(),
...
fn test_empty_hand_comparison() -> Result<(), PokerError> {
    ...
    let empty_hand1 = Hand::from_str(Stud, "")?;
    let empty_hand2 = Hand::from_str(Stud, "")?;
    let one_card_hand = Hand::from_str(Stud, "2h")?;
